...
export default function ExpenseDetail ( { navigation, route } ){
  ...
  const [categoriaSelecionadoEdit, setCategoriaSelecionadoEdit] = useState([]); 

  const user_id = database.auth().currentUser.uid;

  function updateInfos(name, desc, valor, date, isSelected, categoriaSelecionado, id){
    database.firestore().collection("expenses").doc(id).update({
      name: name,
      desc: desc,
      valor: valor, 
      date: date, 
  ...
  return(
    <ScrollView style={styles.container}>
      <View  style={styles.grade}>
        ...
        <View style={styles.inputGroup}>  
            <Picker
              selectedValue={categoriaSelecionadoEdit}
              onValueChange={(itemValue) => 
                setCategoriaSelecionadoEdit(itemValue)
              }
            >
              {
                categoria.map(cr => {
      ...
      <View style={styles.button}>
        <Button
          title="Salvar"
          type="outline"
          onPress={() => updateInfos(nameEdit, descEdit, valorEdit, dateEdit, categoriaSelecionadoEdit, isSelectedEdit, idExpense)} 
        />
      </View>  
    </ScrollView>
  )
